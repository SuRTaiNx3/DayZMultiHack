...
namespace UtilsLibrary
{
    public class ProcessMemory
    {
        ...
        /// </summary>
        /// <param name="address">The address of the Vector to read</param>
        /// <returns>The Vector3 read from memory</returns>
        public Vector3 ReadVector3(IntPtr address)
        {
            float x = this.Read<float>(address);
            float z = this.Read<float>(address + 0x4);
            float y = this.Read<float>(address + 0x8);
        ...
        /// </summary>
        /// <param name="address">The address to write the Vector to</param>
        public Vector3 WriteVector3(IntPtr address, float x, float y, float z)
        {
            this.Write<float>(address, x);
            this.Write<float>(address + 0x4, y);
            this.Write<float>(address + 0x8, z);

            return this.ReadVector3(address);
        }
        ...
        public bool GetProcess()
        {
            ...
            if (this.ProcessName != "")
            {
                ...
                this.CurrentProcess.Exited += CurrentProcess_Exited;

                this.Handle = WinAPI.OpenProcess(WinAPI.ProcessAccessFlags.All, false, this.CurrentProcess.Id);
        ...
        public int DllModuleAddress(string dllname)
        {
            try
            {
                ProcessModuleCollection modules = this.CurrentProcess.Modules;
